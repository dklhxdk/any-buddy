...
export async function runInteractive(flags = {}) {
  ...
  if (hasCompanion) {
    ...
    if (newName && newName.length > 200) {
      console.log(chalk.yellow('  Name too long (max 200 characters). Skipping.'));
    ...
    if (newPersonality && newPersonality.length > 10000) {
      console.log(chalk.yellow('  Personality too long (max 10,000 characters). Skipping.'));
    } else if (newPersonality && newPersonality !== currentPersonality) {
      try {
