...
function which(cmd) {
  try {
    const result = execFileSync(IS_WIN ? 'where' : 'which', [cmd], { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
...
export function isClaudeRunning(binaryPath) {
  try {
    ...
    // pgrep exits 1 (throws) when no match — caught by outer try/catch → returns false
